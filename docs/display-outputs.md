...
**State: not working.** The DRM connectors exist (`card1-DP-1`, `card1-DP-2`, `card1-HDMI-A-1`), and
the internal panel proves the DP machinery works, but the external path does not come up yet.
...
| Alt-mode plumbing | `pmic_glink` + `typec`/`ucsi_glink` + `phy-qcom-qmp-combo` (USB-C combo PHYs `fd5000.phy`, `fde000.phy`) |
...
## Current state
    ...
    gcc_usb3_tert_phy_com_aux_clk: WARNING ...            (the combo PHY's aux clock)
...
**What is missing:** the qmp combo PHY needs the HPG PLL programming series for this generation,
which is posted but not merged. Until that lands, DP alt-mode cannot be brought up on this machine, and
there is no local patch worth carrying for it.
...
Not needed once the eDP link trains (which patches/0006+0007 make it do); it is
...
1. Land the qmp combo PHY HPG PLL programming series for this generation.
2. Then check the `pmic_glink` ↔ combo-PHY device links, which the alt-mode connector creation
   depends on.
3. Consider patch 0008 above, whose failure mode (a silent SoC reset after a failed enable) will
   otherwise keep hiding the real error.
